	...
	public static void main(String[] args) {
		...
		Map<EBook.Category, List<EBook>> ebMap = 
				ebooks
					.stream()
					.collect(
						Collectors.groupingBy(EBook::getCategory));
					...
					.collect(
							Collectors
								.groupingBy(
									EBook::getCategory,
									Collectors
									.collectingAndThen(Collectors.maxBy(Comparator.comparingInt(EBook::getPrice)),
									Optional::get)));
					...
					.collect(
							Collectors.groupingBy(
									...
									Collectors.reducing(
											...
											EBook::getPrice,
											Integer::sum)));
		ebMap3.forEach((k,v) -> System.out.println(k + "\n" + v));			
	}
