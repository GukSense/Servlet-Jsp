...
public class Enum {
	...
	public static void main(String[] args) {
		...
		Country[] countries = Country.values(); //  values ->  enum의 요소들을 순서대로 enum타입의 배열로 리턴.  
		for(int i=0; i<countries.length; i++) {
			System.out.println("name: " + countries[i].name() + ", \tindex: " + countries[i].ordinal());	/*ordinal() ->  원소에 열거된 순서를 정수 값으로 리턴
