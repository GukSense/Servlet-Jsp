	...
	public static void main(String[] args) {
		while(true) {
			...
			try {
				...
				System.out.println("문의사항은 000-1234-5678로 연락주세요.\n");
...
class LoginService3 {
	...
	public static void login(User3 user) throws IDMismatchException3, PasswordMismatchException3 {
		try {
			...
			if (!DB_ID.equals(user.getId())) {
				throw new IDMismatchException3("아이디를 잘못입력하셨습니다.");
				
			} else if (!DB_PW.equals(user.getPassword())) {
				throw new PasswordMismatchException3("비밀번호를 잘못입력하셨습니다.");
			} else {
				user.setLogin(true);
			}
		} catch(IDMismatchException3 e) {
			System.out.println("아이디찾기를 원하시면 클릭해주세요.");
			throw e;
		} catch(PasswordMismatchException3 e) {
			System.out.println("비밀번호찾기를 원하시면 클릭해주세요.");
			throw e;
		} finally {
	...
	public static void increaseCount() {
		count++;
		System.out.println("로그인시도 " + count + "회");
	}
}
